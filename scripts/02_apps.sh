...
install_cask_apps() {
  ...
  brew cask install alfred apple-events atom bartender bettertouchtool blockblock dolphin dropbox electron-api-demos firefox fog gifloopcoder gitup imageoptim imitone iterm2 keka lulu phoenix processing protonvpn shotcut steam transmission whatsyoursign wwdc yacreader
...
install_tinyscripts() {
  ...
  brew install cask-repair climergebutton contagem-edp gfv human-media-time labelcolor lovecolor macspoof pedir-gas pinboardbackup pinboardlinkcheck pinboardurlupdate pinboardwaybackmachine podbook progressbar ringtonemaker rtp-download seren trello-purge-archives upload-file workflows-renamer
}
