function install_brew_apps {
  ...
  brew install alfred-add-requested alfred-placeholder-workflows alfred-rebuild-notificator alfred-rebuild-helpers alfred-workflow-update alfred-workflows-renamer app-icon-extract cask-analytics cask-repair cli-accept-button gfv human-media-time labelcolor lossless-compress lovecolor macspoof makeicns mtg-wallpapers pedir-gas pinboardbackup pinboardlinkcheck pinboardurlupdate pinboardwaybackmachine pkg-extract podbook progressbar ringtonemaker rtp-download seren trello-purge-archives upload-file
