function install_brew_apps {
  ...
  brew install aria2 duti exiftool ffmpeg gifski git handbrake hr imagemagick jq kepubify mas massren media-info megatools mkvtoolnix mp4v2 neovim ripgrep rmlint shellcheck trash tree youtube-dl zoxide
  ...
  brew install alfred-add-requested alfred-placeholder-workflows alfred-rebuild-notificator alfred-rebuild-helpers alfred-workflow-update alfred-workflows-renamer app-icon-extract calm-notifications cask-analytics cli-approve-button cli-merge-button gfv human-media-time labelcolor lossless-compress makeicns mtg-wallpapers pedir-gas pinboard-backup pinboard-delete-unread pinboard-link-check pinboard-url-update pinboard-waybackmachine pkg-extract progressbar ringtonemaker rtp-download seren trello-purge-archives upload-file
